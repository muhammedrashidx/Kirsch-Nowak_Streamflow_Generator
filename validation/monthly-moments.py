...
def boxplots(syn, hist, xticks=True, legend=True, loc='upper right'):
  ...
  plt.plot([], c='green', label='Synthetic')
...
S = np.loadtxt('synthetic/' + site + '-100000x1-monthly.csv', delimiter=',') # n_realizations x 12*n_synthetic_years
...
for j in range(2):    
    ...
    #boxplots(S.std(axis=1), H[r].std(axis=1), xticks=False, legend=False)
   ...
   # ax.set_xlabel('Month of Year')
    ...
    plt.savefig('boxplots_&_pvalues.png', dpi = 500)
